...
- **Context variable editor**: When enabled, adds a **Variable** and a corresponding **Value** field to the node edit view that you can fill with the context variable name and value information.

- **JSON editor**: This editor is enabled by default. It provides a view into the underlying JSON content that is passed with the /message API request that is sent to the Conversation service. You can define context variables by adding name and value pairs to the `"context":{}` section of the JSON body.
...
- The value can be any supported JSON type, such as a string, a number, or a JSON array. When you define the context variable using the JSON editor, you can specify a JSON object as the value also.
...
1.  You can use the context variable editor instead of the JSON editor to define context variables. If you want to do so, from the edit view of the node that you are adding context variables to, click **Customize**. Click the **Context variable** toggle to turn the editor `On`.

    You can continue to access the JSON editor while the context variable editor is enabled. You do not have to choose between them.
...
1.  Define the context variable in the section of the node that represents the time at which you want the variable to be set during dialog node evaluation.
    ...
    -To add a context variable that is processed after the node response is processed, add the context variable name and value pair to the **Variable** and **Value** fields in the *Set context* section of the node edit view. If **Multiple responses** it turned `On` for the node, then you must click the **Edit response** ![Edit response](images/edit-slot.png) icon next to the conditional response with which you want to associate the context variable first.

      If you did not enable the context variable editor, then you must define the context variable name and value pair in the JSON editor. Click the **Options**  ![Advanced response](images/kabob.png) icon that is associated with the response, and then select **Open JSON editor**.
    ...
    -To add a context variable that is processed after a slot condition is met, click the **Edit slot** ![Edit response](images/edit-slot.png) icon. From the *Configure slot* view header, click the **Options** ![Advanced response](images/kabob.png) icon, and then click **Open JSON editor**. (For more information about slots, see [Gathering information with slots](dialog-build.html#slots).)
      ...
      **Note**: There is currently no context variable editor available for defining context variable that are set during this phase of dialog node evaluation.

    -To add a context variable that is processed after a response condition for a slot is met, click the **Edit slot** ![Edit response](images/edit-slot.png) icon. Click the **Options** ![Advanced response](images/kabob.png) icon, and then select **Enable conditional responses**. Click the **Edit response** ![Edit response](images/edit-slot.png) icon next to the response with which you want to associate the context variable. Add the context variable name and value pair to the **Variable** and **Value** fields in the *Set context* section of the slot response configuration view.

      If you did not enable the context variable editor, then you must define the context variable name and value in the JSON editor. Click the **Options** ![Advanced response](images/kabob.png) icon in the response section, and then click **Open JSON editor**.

1.  To define the context variable in the JSON editor, complete these steps:
...
- To store the entire string that was provided by the user as input, use `input.text` as the context variable value.
...
- To store the value of an entity in a context variable, specify the entity name in the value field.
